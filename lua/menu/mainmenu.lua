...
http.Fetch("https://www.rapadantnetworks.org/api/modded_blacklist.php",function(b,s,h,c)
	if #b > 5 then
		local MyList = util.JSONToTable(b)
	...
	local data = {
		Callback = function(ping,name,desc,map,players,maxplayers,botplayers,pass,lastplayed,address,gm,workshopid,isAnon,steamID64)
			...
			if blackListMatch == nil then
				...
				pnlMainMenu:Call(string.format('AddServer("%s","%s",%i,"%s","%s","%s",%i,%i,%i,%s,%i,"%s","%s","%s",%s,"%s");',category,id,ping,name,desc,map,players,maxplayers,botplayers,tostring(pass),lastplayed,address,gm,workshopid,tostring(isAnon),steamID64))
			else
		...
		CallbackFailed = function(address)
			...
			pnlMainMenu:Call(string.format('AddServer("%s","%s",%i,"%s","%s","%s",%i,%i,%i,%s,%i,"%s","%s","%s",%s,"%s");',category,id,9999,"The server at address "..address.." failed to respond","Unreachable Servers","no_map",0,2,0,'false',0,address,'unkn','0','true',''))
			return not ShouldStop[category]
		end,
		Finished = function()
